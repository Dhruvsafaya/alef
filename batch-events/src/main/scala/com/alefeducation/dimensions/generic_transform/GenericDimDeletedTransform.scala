...
class GenericDimDeletedTransform(val session: SparkSession, val service: SparkBatchService, val serviceName: String) {
  def transform(): Option[Sink] = {
    import com.alefeducation.util.BatchTransformerUtility._

    service.readOptional(getList(serviceName, "deleted-source").head, session,
      extraProps = List(("mergeSchema", "true"))).map(_.transformForDelete(
      getNestedMap(serviceName, "column-mapping"),
      getNestedString(serviceName, "entity"),
      getList(serviceName, "unique-ids")
    )).map(DataSink(getList(serviceName, "deleted-sink").head, _))

  }
}

@deprecated("Please use GenericSCD1Transform instead")
object GenericDimDeletedTransform {
  def main(args: Array[String]): Unit = {
    val session: SparkSession = SparkSessionUtils.getSession(args(0))
    val service = new SparkBatchService(args(0), session)
    val transformer = new GenericDimDeletedTransform(session, service, args(0))
