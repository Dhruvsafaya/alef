...
object AdaptivePracticeProgressTransformer extends CommonTransformer {
  ...
  override def getSelectQuery(schema: String): String = {
    ...
    s"SELECT $pkCol $from WHERE ${filterCondition()} ORDER BY $pkCol LIMIT $QUERY_LIMIT"
  }
  ...
  override def getInsertFromSelectQuery(schema: String, ids: List[Long]): String = {
    ...
    s"""
       |INSERT INTO $schema.fact_adaptive_practice_progress ($insCols)
       |SELECT $selCols
  ...
  override def getStagingTableName(): String = "staging_adaptive_practice_progress"

  private def filterCondition():String = {
    """
     | (staging.question_skill_id is null or dloq.lo_dw_id is not null) and
     | (staging.next_question_skill_id is null or dlonq.lo_dw_id is not null)
     """.stripMargin
  }
  ...
  private def fromStatement(schema: String): String = {
    ...
    s"""
       ...
       |  LEFT JOIN $schema.dim_learning_objective dloq ON dloq.lo_id = staging.question_skill_id
       |  LEFT JOIN $schema.dim_learning_objective dlonq ON dlonq.lo_id = staging.next_question_skill_id
       |""".stripMargin
  ...
  private def makeColNames(cols: List[String]): String =
    cols
      .map {
        ...
        case "question_skill_dw_id"      => s"\tdloq.lo_dw_id AS question_skill_dw_id"
        case "next_question_skill_dw_id" => s"\tdlonq.lo_dw_id AS next_question_skill_dw_id"
