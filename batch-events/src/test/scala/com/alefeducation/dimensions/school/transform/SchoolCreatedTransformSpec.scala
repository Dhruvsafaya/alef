...
class SchoolCreatedTransformSpec extends SparkSuite {
  ...
  test("school created should transform") {
    ...
    val transformer = new SchoolCreatedTransform(sprk, service)
    ...
    when(service.getStartIdUpdateStatus("dim_school")).thenReturn(1)
    ...
    assert[String](df, "school_name", "school-name-1")
    assert[String](df, "school_content_repository_id", "cr1")
    assert[Int](df, "school_content_repository_dw_id", 3)
    assert[String](df, "school_organization_code", "globus")
    assert[Int](df, "school_organization_dw_id", 5)
    assert[String](df, "school_created_time", "1970-07-14 02:40:00.0")
