...
class AdaptivePracticeProgressTransformerTest extends AnyFunSuiteLike {
  test("should prepare select query") {
    val query = AdaptivePracticeProgressTransformer.getSelectQuery("alefdw")

    val expectedQuery =
      s"""
             |SELECT staging.dw_id FROM alefdw_stage.staging_adaptive_practice_progress staging
             ...
             | LEFT JOIN alefdw.dim_learning_objective dloq ON dloq.lo_id = staging.question_skill_id
             | LEFT JOIN alefdw.dim_learning_objective dlonq ON dlonq.lo_id = staging.next_question_skill_id
             | WHERE (staging.question_skill_id is null or dloq.lo_dw_id is not null) and (staging.next_question_skill_id is null or dlonq.lo_dw_id is not null)
  ...
  test("should prepare insert query") {
    val query = AdaptivePracticeProgressTransformer.getInsertFromSelectQuery("alefdw", List(10, 12, 13))
    val expectedQuery = """
          |INSERT INTO alefdw.fact_adaptive_practice_progress (
          |  dw_id,
          |  event_type,
          |  _trace_id,
          |  tenant_id,
          ...
          |  staging.time_spent,
          |  staging.question_id,
          |  staging.question_skill_id,
          |  dloq.lo_dw_id AS question_skill_dw_id,
          |  staging.question_difficulty_label,
          |  staging.skill_proficiency_tier,
          |  staging.skill_proficiency_score,
          |  staging.answer_score,
          |  staging.time_spent_on_question,
          |  staging.hint_used,
          |  staging.is_answer_correct,
          |  staging.next_question_id,
          |  staging.attempt_number,
          |  staging.next_question_skill_id,
          |  dlonq.lo_dw_id AS next_question_skill_dw_id,
          |  staging.next_question_difficulty_label
          |  FROM alefdw_stage.staging_adaptive_practice_progress staging
          |  JOIN alefdw.dim_tenant t ON t.tenant_id = staging.tenant_id
          |  JOIN alefdw.dim_school s ON s.school_id = staging.school_id
          |  JOIN alefdw_stage.rel_dw_id_mappings c ON c.id = staging.class_id AND c.entity_type='class'
          |  JOIN alefdw_stage.rel_dw_id_mappings p ON p.id = staging.pathway_id AND p.entity_type='course'
