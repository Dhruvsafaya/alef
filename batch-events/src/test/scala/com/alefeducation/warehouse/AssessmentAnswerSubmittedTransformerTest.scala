package com.alefeducation.warehouse

import com.alefeducation.util.StringUtilities.replaceSpecChars
import org.scalatest.funsuite.AnyFunSuite
import org.scalatest.matchers.should.Matchers

class AssessmentAnswerSubmittedTransformerTest extends AnyFunSuite with Matchers {

  val transformer: AssessmentAnswerSubmittedTransformer.type = AssessmentAnswerSubmittedTransformer


  test("should prepare select query") {
    val query = transformer.getSelectQuery("alefdw")

    val expectedQuery =
      s"""
         |SELECT staging.dw_id
         |FROM alefdw_stage.staging_assessment_answer_submitted staging
         |  JOIN alefdw.dim_tenant t ON t.tenant_id = staging.tenant_id
         |  JOIN alefdw.dim_school s ON s.school_id = staging.school_id
         |  JOIN alefdw.dim_grade g ON g.grade_id = staging.grade_id
         |  JOIN alefdw_stage.rel_dw_id_mappings c ON c.id = staging.class_id
         |  JOIN alefdw_stage.rel_user u ON u.user_id = staging.candidate_id
         |  JOIN alefdw.dim_testpart tp ON tp.id = staging.test_level_id and tp.version = staging.test_level_version
         |  JOIN alefdw.dim_testpart_section_association tps ON tps.id = staging.test_level_section_id and tps.question_id = staging.question_id
         |
         |  ORDER BY staging.dw_id LIMIT 60000
         |""".stripMargin

    replaceSpecChars(query) shouldBe replaceSpecChars(expectedQuery)
  }

  test("should prepare insert query") {
    val query = transformer.getInsertFromSelectQuery("alefdw", List(10, 12, 13))
    val expectedQuery =
      """
        |INSERT INTO alefdw.fact_assessment_answer_submitted (
        |	     dw_id,
        |      created_time,
        |      dw_created_time,
        |      date_dw_id,
        |      _trace_id,
        |      tenant_id,
        |      tenant_dw_id,
        |      event_type,
        |      assessment_id,
        |      academic_year_tag,
        |      school_id,
        |      school_dw_id,
        |      grade_id,
        |      grade_dw_id,
        |      candidate_id,
        |      candidate_dw_id,
        |      class_id,
        |      class_dw_id,
        |      grade,
        |      material_type,
        |      attempt_number,
        |      skill,
        |      subject,
        |      language,
        |      test_level_session_id,
        |      test_level_version,
        |      test_level_dw_id,
        |      test_level_section_id,
        |      test_level_section_dw_id,
        |      test_level,
        |      time_spent,
        |      question_id,
        |      question_code,
        |      question_version,
        |      reference_code
        |)
        |SELECT
        |     staging.dw_id,
        |     staging.created_time,
        |     staging.dw_created_time,
        |     staging.date_dw_id,
        |     staging._trace_id,
        |     staging.tenant_id,
        |     t.tenant_dw_id AS tenant_dw_id,
        |     staging.event_type,
        |     staging.assessment_id,
        |     staging.academic_year_tag,
        |     staging.school_id,
        |     s.school_dw_id AS school_dw_id,
        |     staging.grade_id,
        |     g.grade_dw_id AS grade_dw_id,
        |     staging.candidate_id,
        |     u.user_dw_id AS candidate_dw_id,
        |     staging.class_id,
        |     c.dw_id AS class_dw_id,
        |     staging.grade,
        |     staging.material_type,
        |     staging.attempt_number,
        |     staging.skill,
        |     staging.subject,
        |     staging.language,
        |     staging.test_level_session_id,
        |     staging.test_level_version,
        |     tp.dw_id AS test_level_dw_id,
        |     staging.test_level_section_id,
        |     tps.dw_id AS test_level_section_dw_id,
        |     staging.test_level,
        |     staging.time_spent,
        |     staging.question_id,
        |     staging.question_code,
        |     staging.question_version,
        |     staging.reference_code
        |FROM alefdw_stage.staging_assessment_answer_submitted staging
        |  JOIN alefdw.dim_tenant t ON t.tenant_id = staging.tenant_id
        |  JOIN alefdw.dim_school s ON s.school_id = staging.school_id
        |  JOIN alefdw.dim_grade g ON g.grade_id = staging.grade_id
        |  JOIN alefdw_stage.rel_dw_id_mappings c ON c.id = staging.class_id
        |  JOIN alefdw_stage.rel_user u ON u.user_id = staging.candidate_id
        |  JOIN alefdw.dim_testpart tp ON tp.id = staging.test_level_id and tp.version = staging.test_level_version
        |  JOIN alefdw.dim_testpart_section_association tps ON tps.id = staging.test_level_section_id and tps.question_id = staging.question_id
        |
        |WHERE staging.dw_id IN (10,12,13)
        |
        |""".stripMargin

    replaceSpecChars(query) shouldBe replaceSpecChars(expectedQuery)
  }
}
