...
class StudentProficiencyTrackerTransformerTest extends AnyFunSuite with Matchers {
  val transformer: StudentProficiencyTrackerTransformer.type = StudentProficiencyTrackerTransformer
  ...
  test("should prepare select query") {
    ...
    val expectedQuery =
      s"""
         |SELECT staging.dw_id FROM alefdw_stage.staging_student_proficiency_tracker staging
  ...
  test("should prepare insert query") {
    ...
    val expectedQuery =
      """
        |INSERT INTO alefdw.fact_student_proficiency_tracker
        | ( dw_id,
        |  event_type,
        |  _trace_id,
        |  tenant_id,
        ...
        |   staging.status,
        |   staging.previous_proficiency_tier
        |   FROM alefdw_stage.staging_student_proficiency_tracker staging
        |   JOIN alefdw.dim_tenant t ON t.tenant_id = staging.tenant_id
        |   JOIN alefdw.dim_school s ON s.school_id = staging.school_id
        |   JOIN alefdw_stage.rel_dw_id_mappings c ON c.id = staging.class_id AND c.entity_type='class'
        |   JOIN alefdw_stage.rel_dw_id_mappings p ON p.id = staging.pathway_id AND p.entity_type='course'
